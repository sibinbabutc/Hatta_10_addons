...
class report_account_general_ledger(models.AbstractModel):
    ...
    def group_by_account_id(self, line_id):
        ...
        if context.get('account_ids'):
            base_domain.append(('account_id', 'in', context['account_ids']))
    ...
    def _lines(self, line_id=None):
        ...
        for account in sorted_accounts:
            ...
            if account in context['context_id']['unfolded_accounts'] or unfold_all:
                ...
                too_many = False
                if len(amls) > 180 and not context.get('print_mode'):
                    amls = amls[-180:]
                    too_many = True
                used_currency = self.env.user.company_id.currency_id
                for line in amls:
...
class account_context_general_ledger(models.TransientModel):
    ...
    account_ids = fields.Many2many('account.account', relation='account_report_gl_accounts')
    available_account_ids = fields.Many2many('account.account', relation='account_report_gl_available_account', default=lambda s: [(6, 0, s.env['account.account'].search([]).ids)])

    @api.multi
    def get_available_account_ids_names_and_codes(self):
        return [[c.id, c.name, c.code] for c in self.available_account_ids]
