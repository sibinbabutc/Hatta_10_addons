...
class PurchaseOrder(models.Model):
    ...
    discount_type = fields.Selection([('percentage', 'Percentage'), ('amount', 'Amount')], string='Discount Type',
                                     readonly=True, default='amount')
    discount_rate = fields.Float(string='Discount Rate',
                                 readonly=True, states={'draft': [('readonly', False)], 'sent': [('readonly', False)],
                                                        'cost_received': [('readonly', False)],'sale_ready': [('readonly', False)],
                                                        'to_approve': [('readonly', False)]}, default=0.0)
    discount = fields.Monetary(string='Discount', default=0.0,
                               store=True, compute='compute_lines_discount', track_visibility='always')
    total_before_discount = fields.Monetary(string='Total Before Discount',store=True, compute='compute_total_before_discount')
    ...
    def set_lines_discount(self):
        if self.discount_type == 'percentage':
            ...
            if self.discount_rate != 0 and total != 0:
...
class PurchaseOrderLine(models.Model):
    ...
    price = fields.Float(string='Price', store=True, compute='compute_line_price')
